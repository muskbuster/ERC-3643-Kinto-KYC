...
contract KintoKYC is AbstractModuleUpgradeable {
    /// Address of the Kinto KYC contract
    address public kintoKycAddress;
     ...
     */
    event KintoKycAddressUpdated(address indexed _oldAddress, address indexed _newAddress);
    ...
    function initialize(address _initialKycAddress) external initializer {
        ...
        kintoKycAddress = _initialKycAddress;
    }
     ...
     */
    function updateKintoKycAddress(address _newKycAddress) external onlyComplianceCall {
        if (_newKycAddress == address(0)) revert InvalidKycAddress();
        address oldAddress = kintoKycAddress;
        kintoKycAddress = _newKycAddress;
        emit KintoKycAddressUpdated(oldAddress, _newKycAddress);
    }
    ...
    function isKycVerified(address _userAddress) public view returns (bool) {
        ...
        return IKintoKYC(kintoKycAddress).isKYC(_userAddress);
    }
